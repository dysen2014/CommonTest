...
import com.dysen.dialog.draggable.MovingDraggable;
 ...
 */
public class XDialog<X extends XDialog> {
    ...
    public XDialog(Activity activity) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.ICE_CREAM_SANDWICH) {
            new DialogLifecycle(activity, this);
        }
    }

    public XDialog(Application application) {
    ...
    public void show() {
        ...
        try {
            ...
            // 如果当前限定了显示时长
            if (mDuration != 0) {
                new Timer().schedule(new DialogDismissTask(this), mDuration);
            }
            // 如果当前设置了拖拽
            if (mDraggable != null) {
     ...
     * 设置点击事件
     */
    public X setOnClickListener(int id, OnListener.OnClickListener l) {
     ...
     * 设置长按点击事件
     */
    public X setOnLongClickListener(int id, OnListener.OnLongClickListener l) {
        new ViewClickHandler(this, findViewById(id), l);
        // 当前是否设置了不可触摸，如果是就移除掉
        if ((mWindowParams.flags & WindowManager.LayoutParams.FLAG_NOT_TOUCHABLE) != 0) {
            mWindowParams.flags &= ~WindowManager.LayoutParams.FLAG_NOT_TOUCHABLE;
