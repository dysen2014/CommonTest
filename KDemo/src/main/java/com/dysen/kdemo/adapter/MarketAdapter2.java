...
public class MarketAdapter2 extends SuperRecyclerAdapter<CommonBean.BtcQc> {
    ...
    private CommonBean.BtcQc mTickerData;
    ...
    private MagicIndicatorView magicIndicatorView;
    ...
    public MarketAdapter2(Context mCtx) {
        super(mCtx);
        mContext = mCtx;
    }
    ...
    public void convert(SuperRecyclerHolder holder, CommonBean.BtcQc tickerData, int layoutType, final int position) {
        ...
        switch (getmItemViewType()) {
            case TYPE_KLINE://显示K线逻辑处理
                ...
                magicIndicatorView.setiOnListener(new MagicIndicatorView.IOnListener() {
                    @Override
                    public void onItem(int index) {
                        kLineItemName_ls = itemNameList.get(index);
                        if (mClickListener != null) {
//                            final String[][] chartData = chartMap_ls.get(msymbol_market);
//                            if (chartData == null) {
...
//                            }
            ...
            case TYPE_GONE://不显示K线逻辑处理
                ...
                Glide.with(mContext).load("https://s.gw299.com/statics/img/v3/app_icon/icon_default/icon_btc_64.png").apply(RequestOptions.circleCropTransform()).into((ImageView) holder.getViewById(R.id.img_market_type));
...
//        final String[][] chartData = chartMap.get(mTickerData.getSymbol() + "_" + kLineItemName_ls.getTimeInterval());
    ...
    public void putChartMap(String symbol_market, String[][] chartData) {
        ...
        System.out.println("=======map key=====" + symbol_market);
