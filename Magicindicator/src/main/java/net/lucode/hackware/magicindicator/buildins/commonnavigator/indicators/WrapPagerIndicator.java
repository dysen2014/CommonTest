...
public class WrapPagerIndicator extends View implements IPagerIndicator {
    ...
    public void onPageScrolled(int position, float positionOffset, int positionOffsetPixels) {
        ...
        if (mMode == MODE_MATCH_EDGE) {
            //title宽度
            mRect.left =current.mLeft + (next.mLeft - current.mLeft) * mEndInterpolator.getInterpolation(positionOffset);
            mRect.right = current.mRight + (next.mRight - current.mRight) * mStartInterpolator.getInterpolation(positionOffset);
    ...
    public void setMode(int mode) {
        if (mode == MODE_MATCH_EDGE || mode == MODE_WRAP_CONTENT|| mode == MODE_EXACTLY) {
            mMode = mode;
        } else {
